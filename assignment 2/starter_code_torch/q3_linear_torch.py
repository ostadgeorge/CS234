...
class Linear(DQN):
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        if done_mask.item == True:
            q_samp = rewards
        else:
            q_samp = rewards + gamma * torch.max(target_q_values, dim=1)[0]
        vec = F.one_hot(input=actions.to(torch.int64), num_classes=num_actions)
        loss = torch.sum((q_samp - torch.max(q_values * vec, dim=1)[0]) ** 2)
        return loss
    ...
    def add_optimizer(self):
        ...
        self.optimizer = torch.optim.Adam(self.q_network.parameters(), lr=.005)
